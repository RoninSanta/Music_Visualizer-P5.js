//function to draw a breakdown of the Music bass,treble and Mid
function Needles() {
	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				var x = j*this.plotWidth + (j+1)*this.widthSpacer;
				var y = i*this.plotHeight + (i+1)*this.heightSpacer;
				...
				//add on the ticks
                var centreX = (this.plotWidth/2)+(this.plotWidth)*(j) + (j+1)*this.widthSpacer;
